...
ClassType有一个constexpr构造函数(第4行)，可以用作模板参数(第19行)。函数模板getDouble(第13行)也是如此，它只接受double类型。我想强调的是，每次调用函数模板getDouble(第21和22行)都会创建一个新函数getDouble。这个函数是给定double值的完全特化版本。
