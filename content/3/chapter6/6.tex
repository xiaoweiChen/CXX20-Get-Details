...
t.joinable()                                                                & 若线程t仍然是可汇入的，则返回true。                 \\
\begin{tabular}[c]{@{}l@{}}t.get\_id() and \\ std::this\_thread::get\_id()\end{tabular} &
返回线程的id。 \\
std::jthread::hardware\_concurrency()                                       & 表示可以并发运行的线程数。  \\
std::this\_thread::sleep\_until(absTime)                                    & 使线程t进入睡眠状态，直到时间点absTime。           \\
std::this\_thread::sleep\_for(relTime)                                      & 让线程t进入睡眠状态，时间为relTime。           \\
std::this\_thread::yield()                                                  & 允许系统运行另一个线程。                   \\
\begin{tabular}[c]{@{}l@{}}t.swap(t2) and \\ std::swap(t1, t2)\end{tabular} & 交换线程。                                          \\
t.get\_stop\_source() &
\begin{tabular}[c]{@{}l@{}}返回与共享停止状态关联的std::stop\_source对象。\end{tabular} \\
t.get\_stop\_token() &
\begin{tabular}[c]{@{}l@{}}返回与共享停止状态关联的std::stop\_token对象。\end{tabular} \\
...
在主程序中，我启动了nonInterruptible和interruptible两个线程(第13行和第22行)。与不可中断线程不同，可中断线程获得std::stop\_token，并在第26行中使用它来检查是否中断:stoken.stop\_requested()。在停止请求的情况下，Lambda函数返回，因此线程结束，调用interruptible.request\_stop()(第37行)停止请求。但这并不适用于之前的调用nonInterruptible.request\_stop()，所以调用无效。
